...
function TikTokDirectPostOptions({ channelId, value, onChange }) {
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-xl px-3.5 py-2.5 text-[12.5px] text-red-700">
        Could not load this account's posting options — {error}
      </div>
    )
  }
  if (!info || !value) {
    return <div className="text-[12.5px] text-ink-400">Loading TikTok posting options…</div>
  }

  const privacyOptions = info.privacy_level_options?.length ? info.privacy_level_options : ['SELF_ONLY']

  return (
    <div className="bg-ink-950/[0.03] border border-ink-200 rounded-xl p-3.5 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {privacyOptions.map((opt) => (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-[12px] font-semibold border transition-all duration-150 ${
...
function SchedulePicker({ date, time, onUpdate }) {
  ...
  return (
    <div className="bg-brand/3 border border-brand/10 rounded-2xl p-3.5 space-y-3">
      {/* Date row */}
      <div>
        <span className="block font-semibold text-[12px] text-ink-700 mb-1.5">When to post</span>
        <div className="flex items-center gap-2 flex-wrap">
          {quickDates.map((d) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-[13px] font-medium transition-all duration-150 border ${
                date === d.value
          ...
          <div className="relative">
            <input
              ...
              value={date}
              min={today}
              className="bg-white border border-ink-200 rounded-xl px-3 py-1.5 text-[13px] text-ink-700 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
              onChange={(e) => onUpdate('date', e.target.value)}
            />
          </div>
          <span className="text-[12px] text-ink-400 ml-1">{getDateLabel(date)}</span>
        </div>
      </div>

      {/* Time slots */}
      <div>
        <span className="block font-semibold text-[12px] text-ink-700 mb-1.5">Post at</span>
        <div className="flex gap-1.5 flex-wrap">
          {TIME_SLOTS.map((s) => (
            ...
            >
              <div className="text-[13px] font-semibold leading-tight">{s.sub}</div>
              <div className={`text-[10.5px] mt-0.5 ${time === s.time && !showCustomTime ? 'text-ink-700' : 'text-ink-400'}`}>
                {s.label}
              </div>
            </button>
          ))}
          ...
          >
            <div className="text-[13px] font-medium leading-tight">+</div>
            <div className="text-[10.5px] mt-0.5">Custom</div>
          </button>
        </div>
      </div>
      ...
      {showCustomTime && (
        <div className="flex items-center gap-3 animate-fadein">
          <input
            type="time"
            value={time}
            className="bg-white border border-ink-200 rounded-xl px-3 py-2 text-[14px] font-mono text-ink-800 focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
            onChange={(e) => onUpdate('time', e.target.value)}
          />
          <span className="text-[13px] text-ink-500">{formatTime12h(time)}</span>
        </div>
      )}
...
export default function CaptionComps({ comps, selectedChannels, onUpdate }) {
  ...
  return (
    <section className="mb-7">
      <h2 className="mb-2.5 text-[11px] font-bold tracking-[.08em] uppercase text-ink-400">Caption and time</h2>
      <div className="flex gap-2 flex-wrap mb-3">
        <button
          type="button"
          onClick={copyAll}
          className="px-3 py-1.5 rounded-xl bg-ink-50 border border-ink-200 text-ink-600 text-[13px] font-medium hover:bg-ink-100 transition-all duration-150"
        >
          ...
          return (
            <div key={id} className="bg-white border border-ink-100 rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-150">
              <header className="px-4 py-3 border-b border-ink-100 flex items-center gap-3 flex-wrap" style={{ background: `${color}05` }}>
                ...
                <span className="text-[13.5px] font-bold text-ink-800">{P.name}</span>
                <span className="text-[12.5px] text-ink-500">{c.h}</span>
                <span className="ml-auto text-[12px] text-ink-400">{P.as}</span>
              </header>
              <div className="p-4 space-y-3">
                ...
                >
                  <textarea
                    ...
                    className="w-full min-h-[84px] bg-ink-50 border border-ink-300 rounded-lg px-3 py-2 text-[13.5px] leading-relaxed resize-y focus:outline-none focus:border-lime-500 focus:ring-2 focus:ring-lime-500/30"
                    onChange={(e) => onUpdate({ updateField: { id, field: 'cap', value: e.target.value } })}
                  />
                  <div className={`text-right mt-1 font-mono text-[11.5px] ${d.cap.length > P.limit ? 'text-red-600 font-semibold' : 'text-ink-400'}`}>
                ...
                {c.s === 'soon' && (
                  <div className="bg-amber-50 text-amber-800 border border-amber-200 rounded-xl px-3 py-2 text-[12.5px]">
